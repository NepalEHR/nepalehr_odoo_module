...
from odoo import models,osv,netsvc, fields, api, _
...
#LY 0514
class stock_return_reason(models.Model):
...
class StockPicking(models.Model):
    ...
    return_type= fields.Selection([('none', 'Normal'), ('customer', 'Return from Customer'), ('supplier', 'Return to Supplier')], default= 'none',string='Type', required=True, select=True, help="Type specifies whether the Picking has been returned or not.")
    return_reason_id= fields.Many2one('stock.return.reason', string='Return Reason')
...
class StockPickingIn(models.Model):
    ...
    return_type = fields.Selection([('none', 'Normal'), ('customer', 'Return from Customer'), ('supplier', 'Return to Supplier')], string='Type', default= 'none',required=True, select=True, help="Type specifies whether the Picking has been returned or not.")
    return_reason_id = fields.Many2one('stock.return.reason', string='Return Reason')
...
class stock_partial_picking(models.Model):
...
class stock_return_picking(models.Model):
    ...
    return_type = fields.Selection([('none', 'Normal'), ('customer', 'Return from Customer'), ('supplier', 'Return to Supplier')],string= 'Type',compute=_get_return_type, required=True, select=True, help="Type specifies whether the Picking has been returned or not.")
    note =      fields.Text(string='Notes')
    location_id = fields.Many2one('stock.location',string= 'Location', help='If a location is chosen the destination location for customer return (or origin for supplier return) is forced for all moves.')
    return_reason_id = fields.Many2one('stock.return.reason', string='Return Reason')
...
class stock_move_scrap(models.Model):
    ...
    note =  fields.Text(string ='Return reason')
...
#        return res
    ...
    #  store=_store_product_supply_method, 
    product_supply_method =  fields.Char(compute=_get_product_supply_method,  string='Product Supply Method',readonly=True)
    stock_type = fields.Char(compute=_get_stock_type, string='T')
    #   store=_store_stock_type,
    qty_available = fields.Float(compute=_get_qty_available,  string='Quantity Available', readonly=True)
    # store=_store_product_code, 
    product_code = fields.Char(compute=_get_product_code,   string='Product Reference', readonly=True)
    ...
    partner_ref =  fields.Many2one(related='partner_id',  string='Partner Ref')
    picking_return =  fields.Many2one(related='picking_id',  string='Picking Return',  readonly=1)
...
class stock_location(models.Model):
    ...
    location_return =  fields.Boolean(string='For Return ?', help='This location is for return goods?')
...
class stock_warehouse(models.Model):
    ...
    lot_return_id = fields.Many2one('stock.location', string ='Location For Return', domain=[('location_return', '=', 'True')])

class mrp_production(models.Model):
    _inherit = 'mrp.production'

    
    last_exception = fields.Text(  string = 'Last Exception')
